...
public interface PostViewRepository extends ReadOnlyRepository<PostView, Long> {
  ...
  @Query(value = "select * from post_view where create_time between now() - interval '72 HOURS' and now() " +
    "order by view_num desc limit 100", nativeQuery = true)
  List<PostView> findHot100();

  @Query(value = "select * from post_view order by view_num + like_num desc limit 100", nativeQuery = true)
